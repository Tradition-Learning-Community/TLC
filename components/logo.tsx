...
const sizeMap: Record<Exclude<LogoSize, number>, string> = {
  sm: "h-4",
  md: "h-6",
  lg: "h-10",
};

export const Logo = ({ className, uniColor, size = "lg" }: { className?: string; uniColor?: boolean; size?: LogoSize }) => {
    const heightClass = typeof size === "number" ? undefined : sizeMap[size];
    const style = typeof size === "number" ? { height: size } : undefined;

    return (
        <div className="inline-flex items-center justify-center bg-white rounded-lg px-3 py-2">
            <Image
                src="/logotlc.jpeg"
                alt="TLC"
                width={80}
...
export const LogoIcon = ({ className, uniColor, size = "sm" }: { className?: string; uniColor?: boolean; size?: LogoSize }) => {
    const heightClass = typeof size === "number" ? undefined : sizeMap[size as Exclude<LogoSize, number>];
    const style = typeof size === "number" ? { height: size, width: size } : undefined;
    ...
    return (
        <div className="inline-flex items-center justify-center bg-white rounded-lg p-2">
            <Image
                ...
                height={80}
                style={style}
                className={cn('object-contain', heightClass, className)}
                priority={false}
            />
        </div>
    )
